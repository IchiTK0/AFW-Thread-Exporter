...
while True:
    ...
    POST_WIDTH_PX_INPUT = get_input("Enter the desired post display width in pixels:\t\t(Default: Fit width)")
    POST_ALIGNMENT_INPUT = get_input(f"Enter the desired post alignment:\t\t\t(Options:left,center,right | Default:{DEFAULT_POST_ALIGNMENT})")
    OUTPUT_FILENAME_INPUT = get_input(f"Enter the desired output filename:\t\t\t(Default: {DEFAULT_OUTPUT_FILENAME})")
    OUTPUT_FILETYPE_INPUT = get_input(f"Enter the desired output filetype:\t\t\t(Default: {DEFAULT_OUTPUT_FILETYPE})")
